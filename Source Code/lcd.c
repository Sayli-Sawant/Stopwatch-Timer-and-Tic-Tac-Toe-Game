...
// port b data pin;
        // Enable clock for GPIO Port B (data pins)
         SYSCTL_RCGC2_R |= (1<<1);
         // turning on 0-7  (4--5 is data pins) for output
         GPIO_PORTB_DIR_R = (0xFF);
         // Enable digital function for GPIO Port B data pins
         GPIO_PORTB_DEN_R = (0xFF);

         // Enable clock for GPIO Port A (RS and EN pins)
         SYSCTL_RCGC2_R |= (1<<0);
         // Configure RS pin (Register Select) as an output
         GPIO_PORTA_DIR_R |= (1<<rs_pin);
         // Enable digital function for RS pin
         GPIO_PORTA_DEN_R |= (1<<rs_pin);

         // Configure EN pin (Enable) as an output
         GPIO_PORTA_DIR_R |= (1<<lcd_en);
         // Enable digital function for EN pin
